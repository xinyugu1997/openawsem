...
def debye_huckel_term(self, k_dh=4.15*4.184, forceGroup=30, screening_length=1.0, chargeFile=None, coord_ion_file=None):
        ...
        if chargeFile is None:
            ...
            if coord_ion_file:
               ionInfo = np.loadtxt(coord_ion_file)
               for index in range(len(ionInfo)):
                   charge_i = ionInfo[index][0]
                   i = int(ionInfo[index][1]) - 1
               ...
               for index in range(len(ionInfo)):
                   charge_i = ionInfo[index][0]
                   i = int(ionInfo[index][1]) - 1
                   for jndex in range(index+1, len(ionInfo)):
                              charge_j = ionInfo[jndex][0]
                              j = int(ionInfo[jndex][1]) - 1
                              structure_interactions_dh.append([self.cb[i], self.cb[j], [charge_i, charge_j]])
                              print([self.seq[i], self.seq[j], self.cb[i], self.cb[j], [charge_i, charge_j]])
