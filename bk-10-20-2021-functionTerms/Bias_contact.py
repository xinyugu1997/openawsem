...
def contact_value(oa, group=[], k_contact=4.184, z_dependent=False, z_m=1.5, inMembrane=False, membrane_center=0*angstrom, k_relative_mem=1.0, periodic=False, parametersLocation=".", burialPartOn=True, withExclusion=True,
    ...
    for i in range(oa.natoms):
      if (i in group):
        ...
        contact.addParticle([gamma_se_map_1_letter[seq[oa.resi[i]]], oa.resi[i], 0])
    ...
    print("Number of atom: ", len(group))
...
def d_contact_value(oa, cut, k_contact=4.184, z_dependent=False, z_m=1.5, inMembrane=False, membrane_center=0*angstrom, k_relative_mem=1.0, periodic=False, parametersLocation=".", burialPartOn=True, withExclusion=True,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45, forceGroup=19):
    d_cont = CustomCVForce("cont_t - cont_1 - cont_2")

    cont_t = contact_value(oa, group=range(oa.natoms), k_contact=k_contact, z_dependent=z_dependent, z_m=z_m, inMembrane=inMembrane, membrane_center=membrane_center, k_relative_mem=k_relative_mem, periodic=periodic, parametersLocation=parametersLocation, burialPartOn=burialPartOn, withExclusion=withExclusion, gammaName=gammaName, burialGammaName=burialGammaName, membraneGammaName=membraneGammaName, r_min=r_min)
    ...
    cont_1 = contact_value(oa, group=g1, k_contact=k_contact, z_dependent=z_dependent, z_m=z_m, inMembrane=inMembrane, membrane_center=membrane_center, k_relative_mem=k_relative_mem, periodic=periodic, parametersLocation=parametersLocation, burialPartOn=burialPartOn, withExclusion=withExclusion, gammaName=gammaName, burialGammaName=burialGammaName, membraneGammaName=membraneGammaName, r_min=r_min)
    cont_2 = contact_value(oa, group=g2, k_contact=k_contact, z_dependent=z_dependent, z_m=z_m, inMembrane=inMembrane, membrane_center=membrane_center, k_relative_mem=k_relative_mem, periodic=periodic, parametersLocation=parametersLocation, burialPartOn=burialPartOn, withExclusion=withExclusion, gammaName=gammaName, burialGammaName=burialGammaName, membraneGammaName=membraneGammaName, r_min=r_min)

    d_cont.addCollectiveVariable("cont_t", cont_t)
    d_cont.addCollectiveVariable("cont_1", cont_1)
    d_cont.addCollectiveVariable("cont_2", cont_2)
    d_cont.setForceGroup(forceGroup)
    return d_cont


def bias_contact_term(oa, E0, k_cbias, cut, k_contact=4.184, z_dependent=False, z_m=1.5, inMembrane=False, membrane_center=0*angstrom, k_relative_mem=1.0, periodic=False, parametersLocation=".", burialPartOn=True, withExclusion=True,
                gammaName="gamma.dat", burialGammaName="burial_gamma.dat", membraneGammaName="membrane_gamma.dat", r_min=0.45):
    E0_kJ=E0.value_in_unit(kilojoule_per_mole)
    k_cbias_kJ=k_cbias.value_in_unit((kilojoule_per_mole)**(-1))
    cbias = CustomCVForce(f"0.5*{k_cbias_kJ}*(dE-{E0_kJ})^2")
    dE = d_contact_value(oa, cut=cut, k_contact=k_contact, z_dependent=z_dependent, z_m=z_m, inMembrane=inMembrane, membrane_center=membrane_center, k_relative_mem=k_relative_mem, periodic=periodic, parametersLocation=parametersLocation, burialPartOn=burialPartOn, withExclusion=withExclusion, gammaName=gammaName, burialGammaName=burialGammaName, membraneGammaName=membraneGammaName, r_min=r_min)
    cbias.addCollectiveVariable("dE", dE)
    print("Contact bias term ON, E, k = ", E0, k_cbias)
    return cbias
