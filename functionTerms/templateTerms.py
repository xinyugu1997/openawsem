...
def fragment_memory_term(oa, k_fm=0.04184, frag_file_list_file="./frag.mem", npy_frag_table="./frag_table.npy",
        ...
        for i in range(len(frag)):
            for j in range(i, len(frag)):
                ...
                try:
                    ...
                    print("Frags mem file doesn't match", i_type, j_type)
                    continue
    ...
    print('k_fm=' + str(k_fm) + '\n' +'#interaction:' +str(len(interaction_list)))
    return fm
...
def read_amhgo_structure(oa, pdb_file, amhgo_min_seq_sep=4, amhgo_contact_threshold=0.8*nanometers, amhgo_well_width=0.1):
    structure_interactions = []
...
#    residues = [x for x in chain]
    ...
    for chain in chains:
     for x in chain:
        residues.append(x)
    print("amh_go model applied to ", len(residues), "res")
...
def additive_amhgo_term(oa, pdb_file, k_amhgo=4.184, amhgo_min_seq_sep=3, amhgo_contact_threshold=0.8*nanometers, amhgo_well_width=0.1, forceGroup=24):
    import itertools
    # multiply interaction strength by overall scaling
    ...
    # create bonds
    structure_interactions = read_amhgo_structure(oa, pdb_file, amhgo_min_seq_sep, amhgo_contact_threshold, amhgo_well_width=amhgo_well_width)
